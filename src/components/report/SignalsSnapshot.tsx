...
// At-a-glance summary of the Signals of Influence: the influential core
// circle at center (inside a faint "growth audience" ring), the signal boxes
// around it. Round 9: three boxes — the TRUST box retired with the Trust tab
// (its derivation data still exists on the snapshot; only rendering stopped).
// Every entry is derived from a scored section of the report; each box clicks
// through to its signal tab.

export type SignalTabTarget = "motivational" | "behavioral" | "social";
  ...
  return (
    <Card glow>
      ...
      <p className="mb-5 text-xs text-[#6E7681]">
        The Signals of Influence at a glance — every entry traces to a scored finding in the
        full report. Click a signal to go deeper.
      </p>

      {/* Round 9 three-box composition: MOTIVATIONAL spans the left column;
          BEHAVIORAL and SOCIAL stack on the right; core circle centered. */}
      <div
        className="signals-snapshot-grid grid gap-4"
        style={{
          gridTemplateColumns: "1fr auto 1fr",
          gridTemplateAreas: `"motivational circle behavioral" "motivational circle social"`,
        }}
      >
      ...
      <style>{`
        @media (max-width: 640px) {
          .signals-snapshot-grid {
            ...
            grid-template-areas: "circle" "motivational" "behavioral" "social" !important;
          }
        }
      `}</style>
    </Card>
